...
QByteArray QtTelnetAuthNull::authStep(const QByteArray &data)
{
    ...
    uchar buf[8] = {Common::IAC, Common::SB, Common::Authentication,
...
public:
    ...
    void sendCommand(const uchar *command, int length);
...
void QtTelnetPrivate::sendWindowSize()
{
    ...
    short h = windowSize.height();
    short w = windowSize.width();
    const uchar c[9] = { Common::IAC, Common::SB, Common::NAWS,
                        (uchar)w, (uchar)(w >> 8), (uchar)h, (uchar)(h >> 8),
                        Common::IAC, Common::SE
                      };
...
{
    const uchar c[3] = { Common::IAC, (uchar)operation, (uchar)option };
...
void QtTelnetPrivate::sendCommand(const uchar *command, int length)
{
    QByteArray a((char*)command, length);
...
void QtTelnet::sendSync()
{
    ...
    //todo ::send(s, &tosend, 1, MSG_OOB); // Send the DATA MARK as out-of-band
}
